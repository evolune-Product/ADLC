/**
 * Connections — the merged catalogue gallery.
 *
 * This used to be two pages: a narrow "Connections" list (GitHub/GitLab/Jira
 * only, no catalogue awareness) and a separate "Plugins" gallery (~35 systems,
 * catalogue-driven, honest depth badges). Both wrote into the same underlying
 * `connections` table, so the split was two front doors to one resource, and
 * the narrower one had the actual bugs — GitLab routed through the GitHub
 * client, Linear wasn't handled at all. This page is the merged surface: the
 * catalogue gallery is the superset, so it absorbed the narrower page rather
 * than the other way around.
 *
 * The one rule this page enforces: every card states how deeply the
 * connection is actually wired in. A catalogue of forty logos is worth
 * nothing if thirty-five of them only store a token, and a buyer who
 * discovers that in week two of a trial loses more trust than the logo ever
 * earned. So `depth` is a badge, it is explained in a legend at the top, and
 * "verified" says plainly what it does — the credential is really checked
 * against the vendor, and it is available to agents and skills, but no
 * bespoke pipeline behaviour exists for it yet.
 */
...
import { usePlugins } from '@/hooks/useIntegrations'
import { useConnectCatalogItem, useTestConnection } from '@/hooks/useConnections'
...
function ConnectForm({ plugin, onDone }: { plugin: Plugin; onDone: () => void }) {
  const connect = useConnectCatalogItem()
